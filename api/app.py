...
def encrypt():
    try:
        ...
        # Convert key from hex strings to integers (if needed)
        key_bytes = [int(k, 16) if isinstance(k, str) else k for k in key]
        key1 = [0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x97, 0x21, 0x09, 0x9d, 0x8f, 0x62]

        # Key expansion
        round_keys = key_expansion(key_bytes, RCON, 10)
        ...
        return jsonify({'encrypted_state': encrypted_state}), 200
...
def decrypt():
    try:
        ...
        key = data['key']  # Expecting key as a list of hexadecimal values
        ...
        # Flatten ciphertext
        flat_ciphertext = [byte for sublist in ciphertext for byte in sublist]

        # Decrypt ciphertext
        decrypted_state = aes_decrypt(flat_ciphertext, round_keys)

        # Convert state back to string
        decrypted_plaintext = ''.join([chr(byte) for sublist in decrypted_state for byte in sublist])

        # Return the decrypted plaintext
        return jsonify({'decrypted_plaintext': decrypted_plaintext}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400
